package com.github.walkvoid.zone.user.controller;
...
public class RoleController {
    ...
    @PostMapping
    public ApiResult<RoleDTO> add(@RequestBody RoleDTO dto) {
        if (Objects.isNull(dto)) return ApiResult.ok(null);
        Role entity = BeanCopyUtils.copyBean(dto, Role.class);
        roleDAO.insert(entity);
        return ApiResult.ok(BeanCopyUtils.copyBean(entity, RoleDTO.class));
    }

    @Operation(summary = "删除角色")
    @DeleteMapping("/{id}")
    public ApiResult<Boolean> delete(@Parameter(description = "角色ID") @PathVariable("id") Long id) {
        return ApiResult.ok(!Objects.isNull(id) && roleDAO.deleteById(id) > 0);
    }
    ...
    public ApiResult<Boolean> deleteBatch(@RequestBody IdsParam param) {
        ...
        return ApiResult.ok(roleDAO.deleteBatchIds(param.getIds()) > 0);
    }

    @Operation(summary = "更新角色")
    @PutMapping
    public ApiResult<RoleDTO> update(@RequestBody RoleDTO dto) {
        if (Objects.isNull(dto) || Objects.isNull(dto.getId())) return ApiResult.ok(null);
        Role entity = BeanCopyUtils.copyBean(dto, Role.class);
        roleDAO.updateById(entity);
        return ApiResult.ok(BeanCopyUtils.copyBean(roleDAO.selectById(dto.getId()), RoleDTO.class));
    }

    @Operation(summary = "查询角色")
    @GetMapping("/{id}")
    public ApiResult<RoleDTO> getById(@Parameter(description = "角色ID") @PathVariable("id") Long id) {
        return ApiResult.ok(BeanCopyUtils.copyBean(roleDAO.selectById(id), RoleDTO.class));
    }

    @Operation(summary = "条件查询角色列表")
    @GetMapping("/list")
    public ApiResult<List<RoleDTO>> list(RoleDTO dto) {
        Role condition = BeanCopyUtils.copyBean(dto, Role.class);
        return ApiResult.ok(roleDAO.selectList(condition).stream()
    ...
    @GetMapping("/page")
    public ApiResult<PageDTO<RoleDTO>> page(
        ...
        PageDTO<RoleDTO> pageResult = roleDAO.page(pageRequest);
        return ApiResult.ok(pageResult);
    }

    @Operation(summary = "查询所有角色")
    @GetMapping("/all")
    public ApiResult<List<RoleDTO>> all() {
        return ApiResult.ok(roleDAO.selectAll().stream()
                .map(r -> BeanCopyUtils.copyBean(r, RoleDTO.class))
                .collect(Collectors.toList()));
    }
    ...
    public ApiResult<Boolean> assignUserRoles(@Parameter(description = "用户ID") @PathVariable("userId") Long userId,
    ...
    public ApiResult<Boolean> removeUserRole(@Parameter(description = "用户ID") @PathVariable("userId") Long userId,
                                   @Parameter(description = "角色ID") @PathVariable("roleId") Long roleId) {
        return ApiResult.ok(userRoleRelDAO.deleteByUserIdAndRoleId(userId, roleId) > 0);
    }

    @Operation(summary = "移除用户的所有角色")
    @DeleteMapping("/user/{userId}/roles")
    public ApiResult<Boolean> removeUserAllRoles(@Parameter(description = "用户ID") @PathVariable("userId") Long userId) {
        return ApiResult.ok(userRoleRelDAO.deleteByUserId(userId) > 0);
    }

    @Operation(summary = "查询用户拥有的角色")
    @GetMapping("/user/{userId}/roles")
    public ApiResult<List<RoleDTO>> getUserRoles(@Parameter(description = "用户ID") @PathVariable("userId") Long userId) {
        return ApiResult.ok(userRoleRelDAO.selectByUserId(userId).stream()
    ...
    public ApiResult<List<Long>> getRoleUserIds(@Parameter(description = "角色ID") @PathVariable("roleId") Long roleId) {
        return ApiResult.ok(userRoleRelDAO.selectByRoleId(roleId).stream()
    ...
    public ApiResult<Boolean> assignRoleMenus(@Parameter(description = "角色ID") @PathVariable("roleId") Long roleId,
    ...
    public ApiResult<Boolean> removeRoleMenu(@Parameter(description = "角色ID") @PathVariable("roleId") Long roleId,
                                   @Parameter(description = "菜单ID") @PathVariable("menuId") Long menuId) {
        return ApiResult.ok(roleMenuRelDAO.deleteByRoleIdAndMenuId(roleId, menuId) > 0);
    }

    @Operation(summary = "查询角色拥有的菜单ID列表")
    @GetMapping("/{roleId}/menus")
    public ApiResult<List<Long>> getRoleMenuIds(@Parameter(description = "角色ID") @PathVariable("roleId") Long roleId) {
        return ApiResult.ok(roleMenuRelDAO.selectMenuIdsByRoleId(roleId));
    }
}
